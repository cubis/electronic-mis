...
function outputXML($errNum, $errMsgArr, $patientInfoPrep) {
    ...
    if ($errNum == 0) {
        ...
        while ($patientInfo = $patientInfoPrep->fetch(PDO::FETCH_ASSOC)) {
            ...
            $outputString .= "<REMIND>" . $patientInfo['Reminder'] . "</REMIND>\n";
            $outputString .= "</Appointment>";
...
function doService($level) {
     ...
     */
    $qry = "Select * From Appointment INNER JOIN Doctor WHERE FK_PatientID = 
            (Select PK_PatientID FROM Patient WHERE FK_member_id = 
            (SELECT PK_member_id FROM Users WHERE UserName = '" . $_GET['u'] . "')) 
                AND Appointment.FK_DoctorID = Doctor.PK_DoctorID";
